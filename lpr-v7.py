...
def detect(save_img=False):
    ...
    lplate = None
    lp_num = None
    source, view_img, save_txt, imgsz, trace = 'captured_video.mp4', opt.view_img, opt.save_txt, opt.img_size, not opt.no_trace
...
################################################## Characters ##########################################################
                            ...
                            for path, imgchar, im0s, vid_cap in dataset3:
                                ...
                                for i, det3 in enumerate(pred3):  # detections per image
                                    ...
                                    # Print results
                                    for c3 in det3[:, -1]:
                                        sc += f"{names3[int(c3)]}{''}"  # add to string
                                        lp_num = '' + sc
...
##############################################################################################################
            ...
            print(f'{lp_num}  ---Done. ({(1E3 * (t2 - t1)):.1f}ms) Inference, ({(1E3 * (t3 - t2)):.1f}ms) NMS')
            t=time.localtime()
            logg_time = time.strftime("%H:%M:%S %p",t)

            log_file = "Logging_File.csv"
            with open(log_file, 'r') as f:
                dataf= pd.DataFrame({"Vehicle_Type":[vehicle],"Licence_Number":[lp_num],"Logging_Time":logg_time})
